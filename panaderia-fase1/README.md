...
## Fase 1 incluida: Catálogo y Producción
- `/catalogo` (Admin): alta de productos por categoría, cambio de precio con
  historial (RF-02.2) y activar/desactivar.
- `/precios` (panadero y atención al cliente): lista de consulta, solo lectura.
- `/produccion` (panadero y Admin): registro de coches con filas dinámicas
  (producto, latas, panes por lata, mermas), totales e ingreso estimado en vivo,
  y listado de coches valorados con el precio vigente del día de cada hornada.
- La ganancia neta por coche aparecerá cuando la Fase 3 aporte los costos de insumos.
